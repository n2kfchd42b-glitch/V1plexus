-- ============================================================================
-- FIX append_audit_entry AMBIGUOUS COLUMN REFERENCE (canonical_version build)
--
-- Migration 20260418000002_canonical_version.sql added the p_canonical_version
-- parameter and the canonical_version column to the INSERT, but dropped the
-- table-alias qualifiers introduced by 20260417000001. This reintroduced the
-- 42702 ambiguity: bare `entry_hash` in the SELECT body is ambiguous between
-- the RETURNS TABLE output column and the audit_logs table column.
--
-- Fix: restore `al.` qualification on all SELECT column references while
-- keeping the p_canonical_version parameter and canonical_version INSERT.
-- Behaviour is otherwise identical to 20260418000002.
-- ============================================================================
