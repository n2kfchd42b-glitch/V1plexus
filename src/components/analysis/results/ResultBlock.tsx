...
// A canvas block is either a built-in analysis result or the output of a
// custom R run from the code lane.
export type CanvasBlock =
  | { kind: 'analysis'; id: string; createdAt: number; analysisType: AnalysisType; title: string; result: AnalysisResult }
  | { kind: 'code'; id: string; createdAt: number; title: string; source: string; output: string; images: ImageBitmap[]; error: string | null }
...
// Draw a captured WebR plot (ImageBitmap) into a canvas element.
function BitmapImage({ image }: { image: ImageBitmap }) {
  ...
  return <canvas ref={ref} className="max-w-full h-auto rounded-md border" style={{ borderColor: 'var(--border-row)' }} />
}
...
export function ResultBlock({ block, onRemove, defaultExpanded = true }: Props) {
  ...
  const isError = block.kind === 'analysis' ? !!block.result.summary?.error : !!block.error
    ...
    >
      ...
      <div className="flex items-center gap-2 px-4 py-2.5" style={{ borderBottom: expanded ? '1px solid var(--border-row)' : 'none' }}>
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold truncate" style={{ color: 'var(--text-primary)' }}>{block.title}</p>
        </div>
        {isError && (
      ...
      {expanded && (block.kind === 'code' ? <CodeBody block={block} /> : <AnalysisBody block={block} />)}
    </div>
  )
}

// ── Built-in analysis result ───────────────────────────────────────────────
function AnalysisBody({ block }: { block: Extract<CanvasBlock, { kind: 'analysis' }> }) {
  const { t } = useLocale()
  const [tab, setTab] = useState<'results' | 'tables'>('results')
  const { result, analysisType } = block

  if (result.summary?.error) {
    return (
      <div className="px-4 py-3">
        <p className="text-xs leading-relaxed" style={{ color: 'var(--status-error-text)' }}>{String(result.summary.error)}</p>
      </div>
    )
  }

  const findings = getKeyFindings(result, analysisType)
...
// ── Custom R code output ────────────────────────────────────────────────────
function CodeBody({ block }: { block: Extract<CanvasBlock, { kind: 'code' }> }) {
  const { source, output, images, error } = block
  return (
    <div className="px-5 py-4 space-y-4">
      {/* The R that produced this block */}
      <pre className="text-[11px] leading-relaxed overflow-x-auto rounded-md p-3" style={{ background: 'var(--bg-inset)', color: 'var(--text-secondary)', fontFamily: 'var(--font-mono)' }}>
        {source}
      </pre>
      ...
      {images.length > 0 && (
        <div className="space-y-3">
          {images.map((img, i) => <BitmapImage key={i} image={img} />)}
        </div>
      )}

      {/* Console output */}
      {output.trim().length > 0 && (
        <pre className="text-[11px] leading-relaxed overflow-x-auto rounded-md p-3 whitespace-pre-wrap" style={{ background: 'var(--bg-app)', border: '1px solid var(--border-row)', color: 'var(--text-primary)', fontFamily: 'var(--font-mono)' }}>
          {output}
        </pre>
      )}

      {error && (
        <pre className="text-[11px] leading-relaxed overflow-x-auto rounded-md p-3 whitespace-pre-wrap" style={{ background: 'var(--status-error-bg)', border: '1px solid var(--border-status-error)', color: 'var(--status-error-text)', fontFamily: 'var(--font-mono)' }}>
          {error}
        </pre>
      )}

      {!error && output.trim().length === 0 && images.length === 0 && (
        <p className="text-xs text-[var(--text-tertiary)]">Ran with no console output or plots.</p>
      )}
    </div>
  )
}
