...
export function MetaAnalysisConfig({ config, onChange, onRun, loading, columns }: Props) {
  return (
    <div className="space-y-4">
      <VariableSelector label="Effect Size Column (OR, RR, MD, etc.)" value={(config.effectSizeVar as string) ?? ''} onChange={v => onChange({ ...config, effectSizeVar: v })} columns={columns} required />
      <VariableSelector label="Standard Error Column" value={(config.seVar as string) ?? ''} onChange={v => onChange({ ...config, seVar: v })} columns={columns} placeholder="Or provide CI columns below" />
      <VariableSelector label="CI Lower Bound Column (alternative to SE)" value={(config.ciLowVar as string) ?? ''} onChange={v => onChange({ ...config, ciLowVar: v })} columns={columns} placeholder="Optional if SE provided" />
      <VariableSelector label="CI Upper Bound Column" value={(config.ciHighVar as string) ?? ''} onChange={v => onChange({ ...config, ciHighVar: v })} columns={columns} placeholder="Optional if SE provided" />
