...
export function SpatialConfig({ config, onChange, onRun, loading, columns }: Props) {
  return (
    <div className="space-y-4">
      ...
      <VariableSelector label="Value Variable (cases, rates, prevalence)" value={(config.valueVariable as string) ?? ''} onChange={v => onChange({ ...config, valueVariable: v })} columns={columns} required />
      <VariableSelector label="Population Variable (optional — for rates)" value={(config.populationVariable as string) ?? ''} onChange={v => onChange({ ...config, populationVariable: v })} columns={columns} placeholder="Optional" />
