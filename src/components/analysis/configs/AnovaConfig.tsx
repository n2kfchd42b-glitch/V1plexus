...
export function AnovaConfig({ config, onChange, onRun, loading, columns }: Props) {
  return (
    <div className="space-y-4">
      <VariableSelector label="Dependent Variable (numeric)" value={(config.dependent as string) ?? ''} onChange={v => onChange({ ...config, dependent: v })} columns={columns} allowedTypes={['numeric']} required />
      <VariableSelector label="Factor 1 (group variable)" value={(config.factor1 as string) ?? ''} onChange={v => onChange({ ...config, factor1: v })} columns={columns} allowedTypes={['categorical', 'binary']} required />
      <VariableSelector label="Factor 2 (optional — makes Two-Way ANOVA)" value={(config.factor2 as string) ?? ''} onChange={v => onChange({ ...config, factor2: v })} columns={columns} allowedTypes={['categorical', 'binary']} placeholder="Leave empty for One-Way ANOVA" />
