...
export function OrdinalConfig({ config, onChange, onRun, loading, columns }: Props) {
  ...
  return (
    <div className="space-y-4">
      <VariableSelector label="Ordinal Outcome Variable" value={(config.outcome as string) ?? ''} onChange={v => onChange({ ...config, outcome: v })} columns={columns} allowedTypes={['categorical', 'numeric']} required />
